<script>
    var roleIndexUrl ="{{route('namespace.index')}}",//列表数据
        roleChangeStatusUrl="{{route('namespace.status')}}",
        roleDelUrl = "{{route('namespace.del')}}",
        roleAddUrl = "{{route('namespace.add')}}",
        roleSaveUrl = "{{route('namespace.save')}}",
        doAddUrl = "{{route('namespace.doAdd')}}",
        doSaveUrl="{{route('namespace.doSave')}}",
        modeName = "命名空间"
</script>
    ...
    layui.use(['form','layer','laydate','table','laytpl'],function () {
        ...
        var roleTable = table.render({
            ...
            cols : [[
                ...
                {field: 'space', title: '命名空间', width:220},
